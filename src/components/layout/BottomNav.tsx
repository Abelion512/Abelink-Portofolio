...
import { Home, FolderOpen, Trophy, Layers, Search, User } from "lucide-react";
...
const ITEMS = [
  ...
  { href: "/about", icon: User, label: "nav.about" },
];
      ...
      <nav className="flex items-center gap-1 p-1.5 glass-liquid rounded-full border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        ...
        <Link
          ...
          className={`relative p-3.5 rounded-full transition-all active:scale-95 ${
            pathname === "/" ? "text-primary bg-primary/10 border border-primary/20" : "text-text-secondary"
          }`}
        >
        ...
        <Link
          href="/projects"
          className={`relative p-3.5 rounded-full transition-all active:scale-95 ${
            pathname === "/projects" ? "text-primary bg-primary/10 border border-primary/20" : "text-text-secondary"
          }`}
        >
          <FolderOpen size={22} strokeWidth={pathname === "/projects" ? 2.5 : 2} />
        </Link>
