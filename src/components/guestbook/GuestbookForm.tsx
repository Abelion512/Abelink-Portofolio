          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-3 flex gap-2">
              <input
                ...
                className="flex-1 bg-base/40 border border-border rounded-xl px-4 py-3 outline-none focus:border-primary/50 transition-all text-sm"
                required
              />
              <button
                type="submit"
                disabled={isSubmitting}
                className="px-6 py-3 bg-primary text-white rounded-xl font-bold flex items-center gap-2 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                <span className="hidden sm:inline">{t("guestbook.send")}</span>
              </button>
            </div>
          </div>
        </form>
        ...
        <AnimatePresence initial={false}>
          {entries.map((entry, idx) => (
            <motion.div
              ...
              className="glass border border-border bg-surface/5 p-6 rounded-3xl relative overflow-hidden group hover:border-primary/30 transition-all"
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-text-primary text-sm">{entry.user_name}</span>
                </div>
